...
class ReviewComplaintMarketHelper {
  static Future<dynamic> fetchReviewComplaint(
    ...
    try {
      String url = equipmentComplaintType == EquipmentComplaintType.normal
          ? APIs.getReviewComplaintApi
          : equipmentComplaintType == EquipmentComplaintType.marketing
          ? APIs.pendingAcknowledgeApi
          : APIs.getReviewComplaintITApi +
  ...
  static Future<dynamic> submit({
    ...
    try {
      String url = equipmentComplaintType == EquipmentComplaintType.normal ?
         APIs.addReviewComplaintApi
          : APIs.addReviewComplaintITApi;
      var json = {
        ...
        "codeGroup":
            codeGroupData.code != null ? codeGroupData.id.toString() : "",
      ...
      if (files[0].path.isNotEmpty) {
        filesList.add(
            FileModel(name: "file", file: files[0], keyName: "attachFile"));
      }
...
/*        await NotificationHelper.sendNotification(
  ...
  static Future<dynamic> stationCloseApi({
    required BuildContext context,
    required ComplaintMarketModel complaintMarketModel,
    ...
    try {
      ...
      String url = APIs.stationCloseApi;
      var json = {
        "complaintId": complaintMarketModel.id != null ? complaintMarketModel.id.toString() : "",
